...
class CustomZoomTool(ZoomTool):
    ...
    def get_access_token(self) -> str:
        ...
        try:
            response = requests.post(self.token_url, headers=headers, data=data, auth=(
...
def create_scheduler_agent() -> Optional[Agent]:
    # Ensure Zoom credentials are provided before creating the tool.
    if not (st.session_state.zoom_account_id and st.session_state.zoom_client_id and st.session_state.zoom_client_secret):
        st.error("Zoom credentials are missing. Please provide Zoom Account ID, Client ID, and Client Secret in the sidebar.")
        return None
    ...
    return Agent(
        ...
        instructions=[
            ...
            "Schedule interviews during business hours (9 AM - 5 PM PKT)",
...
def schedule_interview(scheduler: Agent, candidate_email: str, email_agent: Agent, role: str) -> None:
    ...
    try:
        ...
        # Schedule interview for the next day at 11 AM PKT
        tomorrow_pkt = current_time_pkt + timedelta(days=1)
        interview_time = tomorrow_pkt.replace(
            hour=11, minute=0, second=0, microsecond=0)
        formatted_time = interview_time.isoformat()
        ...
        meeting_response = scheduler.run(
            ...
            Important Notes:
            - The meeting must be between 9 AM - 5 PM PKT
            - Use PKT (UTC+5) timezone for all communications
        ...
        email_agent.run(
            ...
            Important:
            - Clearly specify that the time is in PKT (Pakistan Standard Time)
        ...
        st.success(
            "Interview scheduled successfully! Check your email for details.")
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        api_key = st.text_input("OpenAI API Key", type="password",
                                value=st.session_state.openai_api_key, help="Get your API key from platform.openai.com")
        if api_key:
        ...
        zoom_client_secret = st.text_input(
            "Zoom Client Secret", type="password", value=st.session_state.zoom_client_secret)

        st.subheader("Email Settings")
        email_sender = st.text_input(
            "Sender Email", value=st.session_state.email_sender, help="Email address to send from")
        email_passkey = st.text_input("Email App Password", type="password",
                                      value=st.session_state.email_passkey, help="App-specific password for email")
        company_name = st.text_input(
            "Company Name", value=st.session_state.company_name, help="Name to use in email communications")
    ...
    # Role selection
    role = st.selectbox("Select the role you're applying for:", [
    ...
    if resume_file:
        ...
        with col2:
            st.download_button(label="📥 Download", data=resume_file,
                               file_name=resume_file.name, mime="application/pdf")
                    ...
                    if is_selected:
                        ...
                        with st.spinner("Sending feedback email..."):
                            try:
                                ...
                                logger.error(
                                    f"Error sending rejection email: {e}")
                                st.error(
            ...
            with st.spinner("🔄 Processing your application..."):
                try:
                    ...
                    if scheduler_agent is None:
                        st.error(
                            "Scheduler agent could not be created due to missing Zoom credentials.")
                        return
